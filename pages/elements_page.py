...
class TestButtonsPage(BasePage):
    locators = TestButtonsPageLocators()
...
class UploadAndDownloadPage(BasePage):
    ...
    def upload_file(self):
        ...
        self.element_is_present(self.locators.UPLOAD_FILE_INPUT).send_keys(path)
        os.remove(path)
        text = self.element_is_present(self.locators.UPLOADED_FILE).text
        return text.split('\\')[-1], path.split('\\')[-1]

    def download_file(self):
        link = self.element_is_present(self.locators.DOWNLOAD_FILE).get_attribute('href')
        link_b = base64.b64decode(link)
        path_name_file = rf'C:\Users\chest\PycharmProjects\python_test\filetest{random.randint(0, 10)}.jpeg'
